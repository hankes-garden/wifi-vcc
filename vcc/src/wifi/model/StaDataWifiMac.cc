...
NS_OBJECT_ENSURE_REGISTERED(StaDataWifiMac);

TypeId StaDataWifiMac::GetTypeId(void)
{
	static TypeId tid =
			TypeId("ns3::StaDataWifiMac").SetParent<RegularDataWifiMac>().AddConstructor<
					StaDataWifiMac>().AddAttribute("ProbeRequestTimeout",
					"The interval between two consecutive probe request attempts.",
					TimeValue(Seconds(0.05)),
					MakeTimeAccessor(&StaDataWifiMac::m_probeRequestTimeout),
					MakeTimeChecker()).AddAttribute("AssocRequestTimeout",
					"The interval between two consecutive assoc request attempts.",
					TimeValue(Seconds(0.5)),
					MakeTimeAccessor(&StaDataWifiMac::m_assocRequestTimeout),
					MakeTimeChecker()).AddAttribute("MaxMissedBeacons",
					"Number of beacons which much be consecutively missed before "
							"we attempt to restart association.",
					UintegerValue(10),
					MakeUintegerAccessor(&StaDataWifiMac::m_maxMissedBeacons),
					...
					BooleanValue(false),
					MakeBooleanAccessor(&StaDataWifiMac::SetActiveProbing),
					MakeBooleanChecker()).AddTraceSource("Assoc",
					"Associated with an access point.",
					MakeTraceSourceAccessor(&StaDataWifiMac::m_assocLogger)).AddTraceSource(
					"DeAssoc", "Association with an access point lost.",
					MakeTraceSourceAccessor(&StaDataWifiMac::m_deAssocLogger));
	return tid;
}

StaDataWifiMac::StaDataWifiMac() :
...
void StaDataWifiMac::SetMaxMissedBeacons(uint32_t missed)
{
	NS_LOG_FUNCTION (this << missed);
	m_maxMissedBeacons = missed;
}

void StaDataWifiMac::SetProbeRequestTimeout(Time timeout)
{
	NS_LOG_FUNCTION (this << timeout);
	m_probeRequestTimeout = timeout;
}

void StaDataWifiMac::SetAssocRequestTimeout(Time timeout)
{
	NS_LOG_FUNCTION (this << timeout);
	m_assocRequestTimeout = timeout;
}

void StaDataWifiMac::StartActiveAssociation(void)
{
	NS_LOG_FUNCTION (this);
	TryToEnsureAssociated();
}

void StaDataWifiMac::SetActiveProbing(bool enable)
{
	NS_LOG_FUNCTION (this << enable);
	if (enable)
	{
		Simulator::ScheduleNow(&StaDataWifiMac::TryToEnsureAssociated, this);
	}
	else
	{
...
void StaDataWifiMac::SendProbeRequest(void)
{
	NS_LOG_FUNCTION (this);
	WifiMacHeader hdr;
	hdr.SetProbeReq();
	...
	m_probeRequestEvent = Simulator::Schedule(m_probeRequestTimeout,
			&StaDataWifiMac::ProbeRequestTimeout, this);
}

void StaDataWifiMac::SendAssociationRequest(void)
{
	...
	m_assocRequestEvent = Simulator::Schedule(m_assocRequestTimeout,
			&StaDataWifiMac::AssocRequestTimeout, this);
}

void StaDataWifiMac::TryToEnsureAssociated(void)
{
	NS_LOG_FUNCTION (this);
	switch (m_state)
	{
...
void StaDataWifiMac::AssocRequestTimeout(void)
{
	NS_LOG_FUNCTION (this);
...
void StaDataWifiMac::ProbeRequestTimeout(void)
{
	NS_LOG_FUNCTION (this);
	SetState(WAIT_PROBE_RESP);
	SendProbeRequest();
...
void StaDataWifiMac::MissedBeacons(void)
{
	NS_LOG_FUNCTION (this);
...
void StaDataWifiMac::RestartBeaconWatchdog(Time delay)
{
...
bool StaDataWifiMac::IsAssociated(void) const
{
	return m_state == ASSOCIATED;
}

bool StaDataWifiMac::IsWaitAssocResp(void) const
{
	return m_state == WAIT_ASSOC_RESP;
}

void StaDataWifiMac::Enqueue(Ptr<const Packet> packet, Mac48Address to)
{
...
void StaDataWifiMac::Receive(Ptr<Packet> packet, const WifiMacHeader *hdr)
{
...
SupportedRates StaDataWifiMac::GetSupportedRates(void) const
{
...
void StaDataWifiMac::SetState(MacState value)
{
